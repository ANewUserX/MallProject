      ...
      <div class="mainCon">
        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" class="itxt" v-model="goodNum" @change="numChange"/>
                <a href="javascript:" class="plus" @click="goodNum ++">+</a>
                <a href="javascript:" class="mins" @click="goodNum > 1 ? goodNum -- : goodNum = 1">-</a>
              </div>
              <div class="add">
                <a href="javascript:" @click="submitBookes">加入购物车</a>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  computed: {
    ...
    skuImageList() {
      // 防止由于父元素需要请求，传递undefined会导致报错，所以传递一个空数组
  ...
  methods:{
    ...
    numChange(event){
      // 获取表单输入的数字 任何非数字的乘以一都是NaN
      let num = (event.target.value) * 1
      if(isNaN(num) || num < 1){
        this.goodNum = 1
      }else{
        // 避免输入浮点数
        this.goodNum = parseInt(num)
      }
    },
    async submitBookes(){
      // console.log(this.$route.params.skuid,this.goodNum);
      // 进行传参，需要传什么就传什么，需要传递的是一个对象
      try {
        // 传递参数
        await this.$store.dispatch('addOrUpdateShopCar',{skuId:this.$route.params.skuid,skuNum:this.goodNum})
        // 路由跳转 将商品的数据进行本地存储
        // 本地存储：localStorage 即使是关闭浏览器和电脑都不会丢失数据
        // 会话存储：sessionStorage 关闭页面就是数据丢失 两种存储方式都不能存储对象 只能存储字符串
